...
//  private final static String CONTENT_LENGTH = "content-length";
    ...
    public static Frame fromString(String data){
    	...
    	format data    	
    	********************/ 
    	int start=data.indexOf("[\""),end=data.lastIndexOf("\"]");
    	if(start>=0 && end>=start){
    		data=data.substring(start+2,end-2);
    	}
    	//好乱
        List<String> contents = new ArrayList<String>(Arrays.asList(data.split("[\\\\]n")));
        ...
        /******************** 
    	//mofidy by feng,20150605
    	format body (\" body \"\\u00)    	
    	********************/
        if(body!=null){
        	start=body.indexOf("\\\"");
        	end=body.lastIndexOf("\\\"\\u00");
        	if(start!=0)start=-2;
        	if(end==-1)end=body.length();
        	body=body.substring(start+2,end);
        }
